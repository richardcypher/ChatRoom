...
public class ChatServer extends UnicastRemoteObject implements IChatServer {
	...
	public void Remove(IChatClient client) throws RemoteException {
		...
		for (int i = 0; i < clients.size(); i++) {
			...
			clients.get(i).PrintMessage(client.getName() + " leave the room",
					null, i, true);
		}
	}
	...
	public void Add(IChatClient client) throws RemoteException {
		...
		for (int i = 0; i < clients.size(); i++) {
			...
			clients.get(i).PrintMessage(client.getName() + " enter the room",
					null, i, true);
		}
	}
	...
	public void SendMessage(IChatClient client, String message, String des)
		...
		for (int i = 0; i < clients.size(); i++) {
			if (clients.get(i).getName().equals(des)) {
				...
				try {
					...
					clients.get(i).PrintMessage(
							client.getName() + ":" + message, v, source, true);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			} else if (!clients.get(i).getName().equals(client.getName()))
				clients.get(i).PrintMessage(client.getName() + ":" + message,
						v, source, false);
		}
	}
	...
	public void ShowUser(IChatClient client) throws RemoteException {
		String alluser = "";
		for (int i = 0; i < clients.size(); i++)
			alluser = alluser + clients.get(i).getName() + "\n";
		client.PrintMessage(alluser, null, 0, true);
	}
